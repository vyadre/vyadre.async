...
(function () {
    ...
    async.forEach = function(arrArgs, func, callback){
      var ready = 0;
      var tl = arrArgs.length;
      for(var i = 0; i < tl; i++){
        func.call(this, arrArgs[i], function(){
          ready++;
          if(ready === tl){
            callback();
          } 
    ...
    async.map = function(arrArgs, func, callback){
      var ready = 0;
      var tl = arrArgs.length;
      var arrRes = [];
      for(var i = 0; i < tl; i++){
        (function(i){
          func.call(this, arrArgs[i], function(res){
            arrRes[i] = res;
            ready++;
            if(ready === tl){
              callback(arrRes);
